Feature: an user gets the resources shared to them
  ...
  @env-config
  Scenario: check the response while sharing a folder with denied permission role (Personal Space)
    Given using spaces DAV path
  ...
  @env-config
  Scenario: check the response while sharing a folder with denied permission role (Project Space)
    Given using spaces DAV path
    And the administrator has enabled the permissions role "Denied"
    And the administrator has assigned the role "Space Admin" to user "Alice" using the Graph API
    And user "Alice" has created a space "NewSpace" with the default quota using the Graph API
  ...
  @env-config
  Scenario: check the response while sharing a file with denied permission role (Personal Space)
    Given using spaces DAV path
    And the administrator has enabled the permissions role "Denied"
  ...
  @env-config
  Scenario: check the response while sharing a file with denied permission role (Project Space)
    Given using spaces DAV path
    And the administrator has enabled the permissions role "Denied"
    And the administrator has assigned the role "Space Admin" to user "Alice" using the Graph API
    And user "Alice" has created a space "NewSpace" with the default quota using the Graph API
